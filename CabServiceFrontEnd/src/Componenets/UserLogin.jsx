...
export default class UserLogin extends Component {
  ...
  handleLogin(e) {
    ...
    if (this.checkBtn.context._errors.length === 0) {
      axios.post(`http://localhost:7005/userlogin/${this.state.emailId}/${this.state.password}`).then(
        (response) => {
          alert("logged succesfully")
console.log(response)
this.props.history.push({ pathname: '/userdashboard/' + this.state.emailId })
}).catch((error) => {
          // Error
      ...
      <div>
        ...
        <Form id="form1"onSubmit={this.handleLogin} ref={c => { this.form = c; }} style={{ width: "400px", color:"black", marginLeft: "700px", fontFamily: "cursive" }}>
          ...
          <div className="form-group">
            ...
            <Input type="email" id="mail" className="form-control" placeholder="Enter EmailId" value={this.state.emailId} onChange={this.onChangeUsername} validations={[required]} />
          </div>
